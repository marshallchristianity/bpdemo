...
select * from autocomplete where $_POST[column] like '%$_POST[search]%'";
		}else{
		...
		{
		 $sql="select * from autocomplete where ".$_POST['column']." like '".$_POST['search']."%'";
		  echo "<a href='excel.php?query=select * from autocomplete&check=".$_POST['column']."&value=".$_POST['search']."' >Export</a>";
		}
		else
		{
		 $sql="select * from autocomplete";
		  echo "<a href='excel.php?query=".$sql."' >Export</a>";
		 }
		...
		echo "</td><td style=font-size:15px>"; 
		echo "<a href='admin/student.php?data=editauto&page=".$row['id']."'>EDIT</a>"."&nbsp;&nbsp;"."<a href='#' class='btn btn-danger btn-setting' id=".$row['id'].">DELETE</a>";
		echo "</td>";
		echo "</tr>";
		}
	...
	<?php $templates = mysqli_query($link, "select * from mail_tamplates");
			// $row= mysqli_fetch_array($templates); 
			 ?>
	<h2 style="font-family:Britannic Bold"><span style="color:red">STUDENT</span><span style="color:green">MAIL</span></h2>
	...
	<input type="text" name="mail_subject" value="<?php if(isset($_SESSION['subject'])){echo $_SESSION['subject'];}?>">
	 ...
	 <?php include("mailer.html"); ?></textarea>

  
	<input type="submit" name="submit" value="Send Email" onclick="nicEditors.findEditor('area1').saveContent();">
	...
	<div class="modal-body">
		...
		<script>
			...
			$(".btn-setting").click(function(){
				...
				$(".yes").attr("href",'admin/dbfiles/autotable.php?page_id='+page_id+"&table=autocomplete");
