...
<input type="submit" name="reset" value="Reset"/></h2>
		...
		if(isset($_POST['searching'])){
			...
			$statement = "`shinedata` WHERE $_POST[column] like '$filter%' union
					select * from shinedata where $_POST[column] like '%$filter' union
					select * from shinedata where $_POST[column] like '%$filter%' order by id DESC";
		...
		if(isset($_POST['reset'])){
			...
			?>
          <?php echo pagination($statement,$per_page,$page,$filter); ?>
        </table>
        <br />
